...
<html lang="eng">
	<head>
		<title>Student List | Caraga State University Student Attendance Record System</title>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('.remove_employee').click(function(){
				...
				var _conf = confirm("Are you sure to delete this Student?");
				if(_conf == true){
					$.ajax({
				...
				$.ajax({
					...
					success:function(resp){
						if(typeof resp !=undefined){
							...
							$('#new_employee .modal-title').html('Edit Student Info')
			...
			$('#new_emp_btn').click(function(){
				...
				$('[name="department"]').val('')
				$('[name="position"]').val('')
				$('#new_employee .modal-title').html('Add New Student Info')
				$('#new_employee').modal('show')
			})
		});
	</script>
